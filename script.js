// IMPORTS ABSOLUTOS (sem import map) -> funciona no GitHub Pages e iPad
...
// OrbitControls usa o próprio canvas
...
const roofMat = new THREE.MeshPhysicalMaterial({
  color:new THREE.Color(getComputedStyle(document.documentElement).getPropertyValue('--roof').trim()),
  roughness:.6, metalness:.2, clearcoat:.6
});
...
const gutterMat = new THREE.MeshStandardMaterial({
  color:new THREE.Color(getComputedStyle(document.documentElement).getPropertyValue('--gutter').trim()),
...
gutterMain.userData = { name: 'Calha principal', desc:'Beiral direito do telhado.' };
...
gutterFront.userData = { name: 'Calha frontal', desc:'Borda frontal do telhado.' };
...
downspout.userData = { name: 'Condutor vertical', desc:'Do telhado ao chão (frente direita).' };
...
pipe1.userData = { name: 'Tubo até filtro', desc:'Tubulação horizontal no piso.' };
...
filter.userData = { name: 'Filtro (areia/carvão)', desc:'Tratamento inicial.' };
...
pump.userData = { name: 'Bomba', desc:'Após o filtro, envia ao reservatório.' };
gStructure.add(pump);
...
tank.userData = { name: 'Reservatório (≈2.000 L)', desc:'Frente direita.' };
gStructure.add(tank);
...
pipe2.userData = { name: 'Tubo para reservatório', desc:'Fecha o circuito.' };
...
dashedPath([ new THREE.Vector3(L/2-2.7-0.45, 0.12, W/2+0.35), new THREE.Vector3(L/2-4.4, 0.12, W/2+0.35) ]);
...
function addLane(x,z,w,l){
  ...
  m.position.set(x, 0.011, z); gWash.add(m);
}
...
function addCone(x,z){
  ...
  body.position.y = 0.28; cone.add(base, body); cone.position.set(x,0,z); gWash.add(cone);
}
...
// ======= INTERAÇÕES / TOOLTIP =======
const selectable = [roof, gutterMain, gutterFront, downspout, pipe1, filter, pump, tank, car, arch, washer,
...
function runTests(){
  ...
  try{
    ...
    out.push(controls.autoRotate ? ok('Auto-rotação habilitável') : bad('Falha auto-rotação'));
    ...
    out.push(typeof window.showTip==='function' ? ok('Função de tooltip disponível') : bad('showTip indisponível'));
